...
export const handler = async (
  ...
  try {
    ...
    if (!lat || !lng || !date) {
      return createErrorResponse(400, 'Missing lat, lng, or date parameters')
    }

    // Call weather service
    const data = await getWeatherService().getHourlyForecast({ lat, lng }, date)

    // Return success response with CORS headers
    return createSuccessResponse(data)
  } catch (error) {
    console.error('Error fetching hourly forecast:', error)
    return createErrorResponse(500, 'Failed to fetch hourly forecast data', error)
  }
}
