  ...
  <div class="location-search">
    ...
    <div class="relative">
      <input
        ...
        class="w-full p-3 pl-10 pr-12 text-white bg-slate-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 placeholder-slate-400"
        @input="onSearchInput"
    ...
    <!-- Loading State -->
    <div v-if="isSearching" class="mt-2 p-3 text-slate-400 text-sm">Searching...</div>

    <!-- Search Results -->
    <div
      v-if="showResults && searchResults.length > 0"
      class="mt-2 bg-slate-700 rounded-lg max-h-60 overflow-y-auto"
    >
      <button
        v-for="(result, index) in searchResults"
        :key="`search-${result.lat}-${result.lng}`"
        :class="[
          'w-full p-3 text-left hover:bg-slate-600 transition-colors border-b border-slate-600 last:border-b-0',
          selectedResultIndex === index ? 'bg-slate-600' : ''
        ]"
        @click="selectLocation(result)"
      >
        <div class="text-white font-medium">{{ result.name }}</div>
        <div class="text-slate-400 text-sm truncate">{{ result.displayName }}</div>
      </button>
    </div>
    ...
    <div
      ...
      class="mt-2 p-3 text-slate-400 text-sm"
    >
      No locations found for "{{ searchQuery }}"
    </div>

    <!-- Error State -->
    <div v-if="searchError" class="mt-2 p-3 text-red-400 text-sm">
      {{ searchError }}
    </div>
    ...
    <div class="mt-3">
      <button
        ...
        class="w-full p-3 text-white bg-blue-600 rounded-lg hover:bg-blue-700 disabled:bg-blue-800 disabled:cursor-not-allowed transition-colors flex items-center justify-center space-x-2"
      >
        <svg
    ...
    <div v-if="recentLocations.length > 0" class="mt-6">
      <h3 class="text-slate-300 text-sm font-medium mb-2">Recent Locations</h3>
      <div class="space-y-1">
        <button
          v-for="location in recentLocations"
          :key="`recent-${location.lat}-${location.lng}`"
          @click="selectRecentLocation(location)"
          class="w-full p-3 text-left bg-slate-700 hover:bg-slate-600 rounded-lg transition-colors"
        >
